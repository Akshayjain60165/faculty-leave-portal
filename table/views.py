...
from .models import (login_table, login_time, num_of_leaves, cur_running,
                              ...
                              cse_hod, mec_hod, ee_hod, hod_dean, director, dean, hodapp, deanapp, dirapp,proffesor,education,course,educationproff,courseproff)
...
def home(request):
    ...
    #p = login_table.objects.all()
        ...
        return render(request,"home.html",{'user': p})

def leaves(request, pro):
    a = cur_running.objects.filter(pro_id = pro)
    b = login_table.objects.get(id = pro)
    if b.dept_name =='CSE':
        c = cse_hod.objects.all()
    if b.dept_name =='EE':
        c= ee_hod.objects.all()
    if b.dept_name =='MEC':
        c=mec_hod.objects.all()
    d = dean.objects.all()
    e = director.objects.all()
    f = num_of_leaves.objects.filter(pro_id = pro)
    g = hodapp.objects.all()
    h = deanapp.objects.all()
    z = dirapp.objects.all()
    return render(request,"leaves.html",{'data':f,'b':b,'c':c,'d':d,'e':e,'a':a,'g':g,'h':h,'z':z})
...
def status(request, pro):
    if request.method == 'POST':
        ...
        f = login_table.objects.get(id = pro)
        x = f.pos
        v = hod_dean.objects.all()
        w = cse_hod.objects.all()
        q = ee_hod.objects.all()
        u = mec_hod.objects.all()
        if f.pos == 'CSEHOD':
            try:
            ...
            if user is None:
                ...
                return render(request,"status.html",{'data':user,'x':x,'v':v,'w':w,'q':q,'u':u})
...
def intro(request,pro):
    p = proffesor.objects.filter(pro_id = pro)
    q = courseproff.objects.filter(pro_id = pro)
    t = educationproff.objects.filter(pro_id = pro)
    return render(request,'pro.html',{'all':p,'course':q,'education':t})
    
def profile(request,pro):
    p = proffesor.objects.filter(pro_id = pro)
    q = courseproff.objects.filter(pro_id = pro)
    t = educationproff.objects.filter(pro_id = pro)
    return render(request,'changeofvalue.html',{'all':p,'course':q,'education':t})

def changeineducation(request,pro):
    if request.method == 'POST' :
        datefrom = request.POST['datefrom']
        #pr = request.POST['id']
        dateto = request.POST['dateto']
        ...
        p = educationproff(pro_id  = pro,datefrom = datefrom, dateto = dateto, institute =  ins, major = major)
        p.save()
        return HttpResponse("done")
...
def changeincourse(request,pro):
    if request.method == 'POST' :
        courseid = request.POST['courseid']
        #pr = request.POST['id']
        coursename = request.POST['coursename']
        p = courseproff(pro_id  = pro, courseid = courseid, coursename = coursename)
        p.save()
        return HttpResponse("done")
...
def deleteineducation(request,pro):
    if request.method == 'POST' :
        #datefrom = request.POST['datefrom']
        #pr = request.POST['id']
        #dateto = request.POST['dateto']
        ...
        educationproff.objects.filter(pro_id  = pro,institute =  institute, major = major).delete()
        return HttpResponse("done")
...
def deleteincourse(request,pro):
    if request.method == 'POST' :
        courseid = request.POST['courseid']
        #pr = request.POST['id']
        coursename = request.POST['coursename']
        courseproff.objects.filter(pro_id  = pro, courseid = courseid, coursename = coursename).delete()
        return HttpResponse("done")
